//! new-version 命令编排：显式创建项目版本。

use std::path::Path;

use super::args::{NewVersionArgs, parse_server_project_version};
use super::{CliError, build_auth_client};

pub async fn run(config: Option<&Path>, args: NewVersionArgs) -> Result<i32, CliError> {
    ...
    let record = auth
        .run_auth(Some(&server_url), |bearer| {
            ...
            async move { client.create_version(&bearer, project_id, &version).await }
        })
        .await?;

    println!("new version created: {}:{}", project, record.version);
    Ok(0)
}
