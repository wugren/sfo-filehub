...
pub fn api_error_response<Resp: Response>(err: &ApiError) -> HttpResult<Resp> {
    ...
    resp.set_content_type("application/json")
        .map_err(|e| SfoError::new(ErrorCode::ServerError, format!("set content type failed: {e}")))?;
    let body = serde_json::to_vec(&json!({ "error": err.code, "message": err.message }))
        .map_err(|e| SfoError::new(ErrorCode::ServerError, format!("encode error body failed: {e}")))?;
    resp.set_body(body);
    Ok(resp)
}
...
pub fn json_ok<Resp: Response, T: Serialize>(status: StatusCode, value: &T) -> HttpResult<Resp> {
    let body = serde_json::to_vec(value)
        .map_err(|e| SfoError::new(ErrorCode::ServerError, format!("encode response failed: {e}")))?;
...
pub fn optional_bearer<Req: Request>(req: &Req) -> Result<Option<String>, ApiError> {
    ...
    let Some(rest) = value.strip_prefix("Bearer ") else {
        return Err(ApiError::unauthorized("authorization header must use Bearer scheme"));
    };
    let rest = rest.trim();
    if rest.is_empty() {
...
pub trait SessionAuth: 'static + Send + Sync {
    ...
    async fn role_of(&self, user_id: crate::model::UserId) -> crate::model::AccountRole;
}
...
impl AuthProvider {
    pub async fn current_principal(&self, bearer: Option<&str>) -> Result<crate::model::Principal, ApiError> {
        match bearer {
            None => Ok(crate::model::Principal::Anonymous),
            Some(bearer) => {
                if let Some(user_id) = self.session_auth.decode_user(bearer).await {
                    let role = self.session_auth.role_of(user_id).await;
                    return Ok(crate::model::Principal::User { user_id, account_role: role });
                }
                match self.token_auth.resolve_token(bearer).await {
                    Some(principal) => Ok(principal),
                    None => Err(ApiError::unauthorized("invalid session or token credential")),
                }
            }
        }
    }

    pub async fn current_principal_req<Req: Request>(&self, req: &Req) -> Result<crate::model::Principal, ApiError> {
        let bearer = optional_bearer(req)?;
        self.current_principal(bearer.as_deref()).await
    }

    pub async fn require_user_req<Req: Request>(&self, req: &Req) -> Result<crate::model::Principal, ApiError> {
        match self.current_principal_req(req).await? {
            ...
            crate::model::Principal::Token { .. } => Err(ApiError::forbidden("user session required for this operation")),
        }
    }

    pub async fn require_user_or_token_req<Req: Request>(&self, req: &Req) -> Result<crate::model::Principal, ApiError> {
        match self.current_principal_req(req).await? {
            crate::model::Principal::Anonymous => Err(ApiError::unauthorized("login required")),
...
pub fn set_download_headers<Resp: Response>(resp: &mut Resp, name: &str) -> Result<(), ApiError> {
    ...
    resp.insert_header(
        ...
        HeaderValue::from_str(&format!("attachment; filename=\"{name}.tar.gz\""))
