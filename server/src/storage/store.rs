...
use super::integrity::{sha256_hex, validate_targz};
...
impl SqliteFileStore {
    pub fn new(db: SqlitePool, data_dir: PathBuf, max_archive_bytes: u64) -> Self {
        Self { db, data_dir, max_archive_bytes }
    }
...
impl super::FileStore for SqliteFileStore {
    async fn ingest(&self, source: UploadStream, expected_sha256: Option<&str>) -> FileStoreResult<FileRecord> {
        if source.len() as u64 > self.max_archive_bytes {
            return Err(FileStoreError {
                kind: super::FileStoreErrorKind::InvalidInput,
                message: format!("archive exceeds max_archive_bytes ({})", self.max_archive_bytes),
            });
        }
        validate_targz(&source)
            .map_err(|message| FileStoreError { kind: super::FileStoreErrorKind::InvalidInput, message })?;
        ...
        tokio::fs::create_dir_all(&self.data_dir)
            .await
            .map_err(|e| FileStoreError { kind: super::FileStoreErrorKind::Io, message: format!("create data dir failed: {e}") })?;
        ...
        let write = async {
            ...
            tokio::fs::write(&temp_path, &source)
                .await
                .map_err(|e| format!("write temp archive failed: {e}"))?;
            let file = tokio::fs::OpenOptions::new()
                .read(true)
                .write(true)
                .open(&temp_path)
                .await
                .map_err(|e| format!("open temp archive failed: {e}"))?;
            file.sync_all()
                .await
                .map_err(|e| format!("sync temp archive failed: {e}"))?;
            drop(file);
            tokio::fs::rename(&temp_path, &final_path)
                .await
                .map_err(|e| format!("rename temp archive failed: {e}"))?;
            Ok::<(), String>(())
        }
        .await;
        if let Err(message) = write {
            ...
            return Err(FileStoreError {
                ...
                message: format!("write archive failed: {message}"),
            });
        }
        let size = source.len() as u64;
        let store = self;
        match sqlx::query("INSERT INTO files (id, rel_path, sha256, size, created_at) VALUES (?, ?, ?, ?, ?)")
            .bind(&file_id)
            .bind(&rel_path)
            .bind(&actual_sha256)
            .bind(size as i64)
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                Err(FileStoreError { kind: super::FileStoreErrorKind::Db, message })
            }
        }
    }
    ...
    async fn open_read(&self, file_id: &FileId) -> FileStoreResult<DownloadStream> {
        let row = sqlx::query("SELECT rel_path FROM files WHERE id = ?")
            ...
            .await
            .map_err(|e| FileStoreError { kind: super::FileStoreErrorKind::Db, message: e.to_string() })?;
        let Some(row) = row else {
            return Err(FileStoreError { kind: super::FileStoreErrorKind::NotFound, message: "file not found".to_string() });
        };
        let rel_path: String = row.try_get("rel_path").map_err(|e| FileStoreError { kind: super::FileStoreErrorKind::Db, message: e.to_string() })?;
        let path = self.safe_path(&rel_path)?;
        tokio::fs::File::open(path)
            .await
            .map_err(|e| FileStoreError { kind: super::FileStoreErrorKind::Io, message: format!("open archive failed: {e}") })
    }
    ...
    async fn discard(&self, file_id: &FileId) -> FileStoreResult<()> {
        let row = sqlx::query("SELECT COUNT(*) AS c FROM version_apps WHERE file_id = ?")
            ...
            .await
            .map_err(|e| FileStoreError { kind: super::FileStoreErrorKind::Db, message: e.to_string() })?;
        let count: i64 = row.try_get("c").map_err(|e| FileStoreError { kind: super::FileStoreErrorKind::Db, message: e.to_string() })?;
        if count > 0 {
            return Err(FileStoreError { kind: super::FileStoreErrorKind::Conflict, message: "file is referenced by a version".to_string() });
        }
        let row = sqlx::query("SELECT rel_path FROM files WHERE id = ?")
            .bind(&file_id.0)
            .fetch_optional(&self.db)
    ...
    async fn gc_orphans(&self, keep: &HashSet<FileId>) -> FileStoreResult<Vec<FileId>> {
        ...
        for row in rows {
            let id: String = row.try_get("id").map_err(|e| FileStoreError { kind: super::FileStoreErrorKind::Db, message: e.to_string() })?;
            let rel_path: String = row.try_get("rel_path").map_err(|e| FileStoreError { kind: super::FileStoreErrorKind::Db, message: e.to_string() })?;
