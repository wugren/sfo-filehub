...
use crate::contract::{api_error_response, json_body, json_ok, path_param, AuthProvider};
use crate::model::{ProjectId, ProjectRole, UserId};

use super::PermissionChecker;

pub fn register<S, Req, Resp>(server: &mut S, checker: Arc<dyn PermissionChecker>, auth: Arc<AuthProvider>)
where
    ...
    Resp: Response,
{
    ...
    server.serve("/api/v1/projects/{project_id}/collaborators", Method::GET, move |req: Req| {
    ...
    server.serve("/api/v1/projects/{project_id}/collaborators/{user_id}", Method::PUT, move |mut req: Req| {
        ...
        async move {
            ...
            let role = crate::api_try!(json_body::<Req, ProjectRoleRequest>(&mut req).await).role;
            match checker.grant_collaborator(&project_id, &principal, &user_id, role).await {
                Ok(()) => {
    ...
    server.serve("/api/v1/projects/{project_id}/collaborators/{user_id}", Method::DELETE, move |req: Req| {
        ...
        async move {
            ...
            let project_id = crate::api_try!(path_param::<Req, ProjectId>(&req, "project_id"));
            let user_id = crate::api_try!(path_param::<Req, UserId>(&req, "user_id"));
            match checker.remove_collaborator(&project_id, &principal, &user_id).await {
                Ok(()) => {
