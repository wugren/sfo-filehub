...
use super::model::{TokenCreateRequest, TokenError, TokenErrorKind, TokenPayload, TokenPrincipal, TokenResult, TokenUpdateRequest};
...
impl SqliteTokenService {
    ...
    fn sign(&self, private_pem: &str, payload: &TokenPayload, expires_at: Option<DateTime<Utc>>) -> TokenResult<String> {
    ...
    fn validate_expiry(&self, expires_at: Option<DateTime<Utc>>) -> TokenResult<()> {
        if let Some(exp) = expires_at {
            ...
            if remaining > MAX_EXPIRY {
                return Err(TokenError::invalid_input("expires_at must be at most 1 year in the future"));
            }
            if remaining.num_seconds() <= 0 {
                return Err(TokenError::invalid_input("expires_at must be in the future"));
            }
        }
        Ok(())
    }
    ...
    fn parse_expires(raw: Option<String>) -> Option<DateTime<Utc>> {
        raw.and_then(|v| DateTime::parse_from_rfc3339(&v).ok()).map(|d| d.with_timezone(&Utc))
    }

    async fn load_token_row(&self, token_id: &TokenId, owner: &UserId) -> TokenResult<Option<(String, String, String, String, String, Option<String>, Option<String>)>> {
    ...
    async fn load_scopes(&self, token_id: &TokenId) -> TokenResult<ScopeSet> {
        ...
        for row in rows {
            ...
            set.insert(Scope::from_str(&raw).map_err(|e: String| TokenError::invalid_input(format!("invalid scope row: {e}")))?);
        }
        Ok(ScopeSet(set))
    }
...
impl super::TokenService for SqliteTokenService {
    async fn create(&self, req: TokenCreateRequest) -> TokenResult<TokenIssued> {
        ...
        let project_scope = req.project_scope.clone().unwrap_or(ProjectScope::All);
        let now = Utc::now();
    ...
    async fn list(&self, owner: &UserId) -> TokenResult<Vec<TokenSummary>> {
        ...
        for row in rows {
            ...
            let project_scope = ProjectScope::from_str(&project_scope_raw)
                .map_err(|e: String| TokenError::invalid_input(format!("invalid project_scope: {e}")))?;
            out.push(TokenSummary {
                token_id,
                name: row.try_get("name")?,
                project_scope,
                scopes: self.load_scopes(&token_id).await?,
                created_at: Self::parse_expires(Some(row.try_get("created_at")?)).unwrap_or_else(Utc::now),
                updated_at: Self::parse_expires(Some(row.try_get("updated_at")?)).unwrap_or_else(Utc::now),
            });
        }
        Ok(out)
    }
    ...
    async fn update(&self, token_id: &TokenId, owner: &UserId, patch: TokenUpdateRequest) -> TokenResult<Option<TokenIssued>> {
        let Some((name, project_scope_raw, public_key_pem, _, _, _, _)) = self.load_token_row(token_id, owner).await? else {
            return Err(TokenError::not_found("token not found"));
        };
        let existing_project_scope = ProjectScope::from_str(&project_scope_raw)
            .map_err(|e: String| TokenError::invalid_input(format!("invalid stored project_scope: {e}")))?;
        let existing_scopes = self.load_scopes(token_id).await?;
        let resign = patch.project_scope.is_some() || patch.scopes.is_some() || patch.expires_at.is_some();
        if !resign {
            if let Some(new_name) = patch.name {
                sqlx::query("UPDATE tokens SET name = ?, updated_at = ? WHERE id = ? AND owner_id = ?")
                    .bind(&new_name)
        ...
        let new_project_scope = patch.project_scope.unwrap_or(existing_project_scope);
        let new_scopes = ScopeSet(patch.scopes.map(|v| v.into_iter().collect()).unwrap_or_else(|| existing_scopes.0.clone()));
        let new_expires = patch.expires_at.unwrap_or(None);
        self.validate_expiry(new_expires)?;
        ...
        let jwt = self.sign(&private_pem, &payload, new_expires)?;
        let mut tx = self.db.begin().await?;
        sqlx::query("UPDATE tokens SET name = ?, project_scope = ?, public_key_pem = ?, updated_at = ? WHERE id = ? AND owner_id = ?")
            .bind(&new_name)
    ...
    async fn rotate(&self, token_id: &TokenId, owner: &UserId) -> TokenResult<TokenIssued> {
        let Some((name, project_scope_raw, _, _, _, _, _)) = self.load_token_row(token_id, owner).await? else {
            return Err(TokenError::not_found("token not found"));
        };
        let _project_scope = ProjectScope::from_str(&project_scope_raw)
            .map_err(|e: String| TokenError::invalid_input(format!("invalid stored project_scope: {e}")))?;
        ...
        sqlx::query("UPDATE tokens SET public_key_pem = ?, updated_at = ? WHERE id = ? AND owner_id = ?")
            .bind(&new_public_pem)
            .bind(Utc::now().to_rfc3339())
            .bind(token_id.0)
            .bind(owner.0)
    ...
    async fn resolve(&self, bearer: &str) -> TokenResult<TokenPrincipal> {
        ...
        let row = sqlx::query("SELECT owner_id, public_key_pem, revoked_at FROM tokens WHERE id = ?")
            .bind(token_id.0)
            .fetch_optional(&self.db)
            .await?;
        ...
        let token_data = jsonwebtoken::decode::<Payload<TokenPayload>>(bearer, &decoding, &validation)
            .map_err(|e| TokenError::not_found(format!("token signature or expiry invalid: {e}")))?;
