...
pub trait VersionService: 'static + Send + Sync {
    ...
    async fn publish_app(&self, project: &ProjectId, version: &str, app: &str, file: FileRecord, actor: &Principal) -> VersionResult<VersionPublish>;
...
impl VersionModule {
    pub async fn init(db: &SqlitePool, checker: Arc<dyn PermissionChecker>) -> Result<Self, String> {
        ...
        Ok(Self {
            service: Arc::new(service::SqliteVersionService::new(db.clone(), checker)),
        })
    }
