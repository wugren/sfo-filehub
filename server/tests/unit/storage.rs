...
use common::{file_record, make_targz, sha256_hex, temp_dir, test_config, assemble};
...
async fn ingest_discard_and_orphan_gc() {
    ...
    let record = state
        .files
        .ingest(archive.clone(), Some(&expected))
        .await
        .expect("ingest");
    ...
    // 非 tar.gz 拒绝
    let bad = vec![0u8; 16];
    assert!(state.files.ingest(bad, None).await.is_err());
    // sha 不一致拒绝
    let zeros = "0".repeat(64);
    assert!(state
        .files
        .ingest(archive.clone(), Some(&zeros))
        .await
        .is_err());
...
async fn gc_keeps_referenced_files() {
    ...
    let a = state.files.ingest(make_targz("a", b"a"), None).await.expect("a");
    let b = state.files.ingest(make_targz("b", b"b"), None).await.expect("b");
