...
async fn dv_full_workflow_with_tokens_and_gc() {
    ...
    assert!(
        state
            .versions
            .list(&project.project_id, &member_principal)
            .await
            .is_err()
    );
    ...
    let f1 = state
        .files
        .ingest(UploadStream::from_bytes(v1.clone()), Some(&sha256_hex(&v1)))
        .await
        .expect("ingest v1");
    state
        .versions
        .create_version(&project.project_id, "1.0.0", &owner_principal)
        .await
        .expect("create v1");
    let out1 = state
        .versions
    ...
    let f2 = state
        .files
        .ingest(UploadStream::from_bytes(v2.clone()), None)
        .await
        .expect("ingest v2");
    state
        .versions
        .create_version(&project.project_id, "2.0.0", &owner_principal)
        .await
        .expect("create v2");
    state
        .versions
    ...
    state
        ...
        .await
        .expect("public");
    let anon_latest = state
        .versions
        .get(&project.project_id, None, &Principal::Anonymous)
        .await
        .expect("anon latest");
    ...
    assert!(
        state
            .versions
            .get(&project.project_id, None, &token_principal)
            .await
            .is_ok()
    );
    assert!(
        state
            .versions
            .create_version(&project.project_id, "9.0.0", &token_principal)
            .await
            .is_err()
    );
    ...
    let rotated = state
        .tokens
        .rotate(&token.token_id, &alice)
        .await
        .expect("rotate");
    assert!(
        to_principal(&state, &format!("Bearer {}", token.jwt))
            .await
            .is_err()
    );
    let rotated_principal = to_principal(&state, &format!("Bearer {}", rotated.jwt))
        .await
        .expect("rotated ok");
    assert!(
        state
            .versions
            .list(&project.project_id, &rotated_principal)
            .await
            .is_ok()
    );
    ...
    assert!(
        small_state
            .files
            .ingest(
                UploadStream::from_bytes(make_targz("big", &vec![0u8; 64])),
                None
            )
            .await
            .is_err()
    ...
    // 删除项目后引用的 files 行与物理归档应立即回收（050）；startup GC
    // 只兜底未被任何版本引用的孤儿文件。
    let orphan = state
        .files
        .ingest(
            UploadStream::from_bytes(make_targz("orphan.txt", b"orphan")),
            None,
        )
        .await
        .expect("ingest orphan");
    let orphan_rows: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM files WHERE id = ?")
        .bind(&orphan.file_id.0)
        .fetch_one(&db)
        .await
        .expect("count orphan rows");
    assert_eq!(
        orphan_rows, 1,
        "unreferenced orphan file row stays for startup GC fallback"
    );
    state
        .projects
        .delete(&project.project_id, &owner_principal)
        .await
        .expect("delete");
    for published in [&f1.file_id, &f2.file_id] {
        let rows: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM files WHERE id = ?")
            .bind(&published.0)
            .fetch_one(&db)
            .await
            .expect("count published rows");
        assert_eq!(rows, 0, "project-deleted file row is gone immediately");
        assert!(
            !dir.join(format!("{}.tar.gz", published.0)).exists(),
            "project-deleted physical archive is gone immediately"
        );
    }
    let removed = state.startup_gc().await.expect("gc after delete");
    let removed_ids: HashSet<String> = removed.iter().map(|f| f.0.clone()).collect();
    assert_eq!(
        removed_ids,
        HashSet::from([orphan.file_id.0.clone()]),
        "startup gc removes only the orphan fallback"
    );
    let after_rows: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM files")
        .fetch_one(&db)
        .await
        .expect("count after gc");
    assert_eq!(after_rows, 0, "startup gc leaves no files rows");
}
...
async fn dv_persistence_across_reopen() {
    ...
    let alice = user_id(&state, "alice").await;
    let p = Principal::User { user_id: alice };
    let project = state
        .projects
        .create(&p, "persist", Visibility::Private)
        .await
        .expect("create");
    ...
    let p2 = Principal::User { user_id: alice2 };
    let list = state2.projects.list(&p2, 100, 0).await.expect("owner list");
    assert_eq!(list.items.len(), 1);
    assert_eq!(list.items[0].project_id, project.project_id);
    assert!(state2.versions.list(&project.project_id, &p2).await.is_ok());
}

async fn to_principal(
    state: &filehub_server::http::AppState,
    bearer: &str,
) -> Result<Principal, String> {
    let value = bearer
        .trim()
        .strip_prefix("Bearer ")
        .unwrap_or(bearer.trim());
